...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(SRWebSocket *me,  NSData *data) {
        CFHTTPMessageAppendBytes(me->_receivedHTTPHeaders, (const UInt8 *)data.bytes, data.length);
        
        if (CFHTTPMessageIsHeaderComplete(self->_receivedHTTPHeaders)) {
            SRFastLog(@"Finished reading headers %@", CFBridgingRelease(CFHTTPMessageCopyAllHeaderFields(me->_receivedHTTPHeaders)));
            [me _HTTPHeadersDidFinish];
        } else {
            [me _readHTTPHeader];
        }
    }];
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(SRWebSocket *me, NSData *newData) {
            if (isControlFrame) {
                [me _handleFrameWithData:newData opCode:frame_header.opcode];
            } else {
                if (frame_header.fin) {
                    [me _handleFrameWithData:me->_currentFrameData opCode:frame_header.opcode];
                } else {
                    // TODO add assert that opcode is not a control;
                    [me _readFrameContinue];
                }
        ...
        switch (eventCode) {
            ...
            case NSStreamEventHasBytesAvailable: {
                ...
                while (self->_inputStream.hasBytesAvailable) {
                    int bytes_read = [self->_inputStream read:buffer maxLength:bufferSize];
